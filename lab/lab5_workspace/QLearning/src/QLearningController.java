...
public class QLearningController extends Controller {
	...
	static final double GAMMA_DISCOUNT_FACTOR = 0.85; /* Must be < 1, small values make it very greedy */
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				double oldValue = Qtable.get(prev_stateaction);
				double newValue = oldValue + alpha(Ntable.get(prev_stateaction))*(previous_reward+GAMMA_DISCOUNT_FACTOR*getMaxActionQValue(new_state)-oldValue) ;
				Qtable.put(prev_stateaction, newValue);
